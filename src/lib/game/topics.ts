...
export type Topic = {
  ...
  blurb: string;
  blurbJa: string;
  /* The book's "Pattern:" box — when to use the method. */
  pattern?: string;
  patternJa?: string;
  /* The method box heading when the book uses another word (e.g. "When to use", "Procedure"). */
  methodTitle?: string;
  methodTitleJa?: string;
  steps: string[];
  stepsJa: string[];
...
export const TOPICS: Topic[] = [
  ...
  },
  {
    ...
    exSteps: (ex, lang) => {
      ...
      const sums = d.slice(1).map((x, i) => `${d[i]}+${x}=${d[i] + x}`);
      const ja = lang === "ja";
      return [
        [`${n} × 11`, "="],
        [ja ? "外側の数字" : "outer digits", `${d[0]} … ${d[d.length - 1]}`],
        [ja ? "となり同士をたす" : "add neighbours", sums.join(",  ")],
        [ja ? "繰り上げて並べる" : "carry and write", `${n * 11}`],
      ];
    },
    example2: () => ({ n: 825 }),
  ...
  },
  {
    ...
    grad: ["#E63A8C", "#FF7AAE"],
    illus: "grid",
    title: "Units Sum 10 and Tens is Same",
    titleJa: "一の位の和が10・十の位が同じ",
    ...
    pattern: "The units digits sum to 10, and the tens digits are same.",
    patternJa: "一の位の和が10で、十の位が同じ。",
    steps: [
  ...
  },
  {
    ...
    grad: ["#FFB020", "#FF7A1A"],
    illus: "grid",
    title: "Tens Sum 10 and Units is Same",
    titleJa: "十の位の和が10・一の位が同じ",
    sutraSa: "Corollary · Ůrdhva-Tiryagbhyām",
    ...
    pattern: "The tens digits sum to 10, and the units digits are same.",
    patternJa: "十の位の和が10で、一の位が同じ。",
    steps: [
  ...
  },
  {
    ...
    grad: ["#17B978", "#79E6A9"],
    illus: "grid",
    title: "Multiplying 2-digit Numbers Starting with 1",
    titleJa: "1で始まる2桁どうしのかけ算",
    sutraSa: "Corollary · Ůrdhva-Tiryagbhyām",
    sutraEn: "12 to 19 – both start with 1",
    sutraEnJa: "12〜19・どちらも1で始まる",
    blurb: "Step 1: multiply the units digits; write units, carry the tens. Step 2: (first number + units of the second) + carry → left part.",
    blurbJa: "手順1：一の位どうしをかけ、一の位を書いて十の位は繰り上げる。手順2：（最初の数＋2番目の数の一の位）＋繰り上げ → 左の部分。",
    pattern: "12 to 19 — both numbers start with 1.",
    patternJa: "12〜19 ―― どちらの数も1で始まる。",
    steps: [
  ...
  },
  {
    ...
    grad: ["#2FA8E0", "#3D6BFF"],
    illus: "ladder",
    title: "Multiplication by 12 to 19",
    titleJa: "12〜19をかける",
    sutraSa: "Ūrdhva-Tiryagbhyām · flag digit",
    sutraEn: "Ultimate & Twice the Penultimate",
    sutraEnJa: "末尾と、その手前の2倍",
    blurb: "The flag digit is the units of 12–19 (the 2…9). Sandwich the number with a 0; from the right, each digit = itself + (flag × the digit to its left), plus any carry. Repeat the same step till the last left digit 0.",
    blurbJa: "旗の数字は12〜19の一の位（2〜9）。数を0ではさみ、右から、各桁＝その桁＋（旗×左どなりの桁）＋繰り上げ。いちばん左の0まで同じ手順をくり返す。",
    steps: [
  ...
  },
  {
    ...
    grad: ["#FFC93C", "#FF9F1C"],
    illus: "squaregrid",
    title: "Square of a Number Ending with 5",
    titleJa: "5で終わる数の2乗",
    sutraSa: "Ekadhikena Pūrvena",
    sutraEn: "10's part × its next number, then 25",
    sutraEnJa: "十の位×次の数、そして25",
    blurb: "Step 1: 10's part × its next number → first part. Step 2: 5 × 5 = 25 → second part (always).",
    blurbJa: "手順1：十の位の部分 ×（その次の数）→ 最初の部分。手順2：5 × 5 = 25 → 2番目の部分（いつも）。",
    steps: [
  ...
  },
  {
    ...
    grad: ["#5865F2", "#8E7BFF"],
    illus: "ladder",
    title: "Subtraction (from 10's Multiples)",
    titleJa: "10・100・1000…からのひき算",
    sutraSa: "Nikhilam Navatáścaramam Daśataḥ",
    sutraEn: "All from 9 and the Last from 10 (Nikhilam)",
    sutraEnJa: "すべて9から、最後は10から（ニキラム）",
    blurb: "Best when the minuend ends in zeros (a power of 10). Subtract each digit from 9, and the last digit from 10. Solve left to right.",
    blurbJa: "引かれる数が0で終わる（10のべき乗の）ときに使う。各桁を9から、最後の桁だけ10から引く。左から右へ解く。",
    methodTitle: "When to use",
    methodTitleJa: "使うとき",
    steps: [
  ...
  },
  {
    ...
    sutraEnJa: "9を取りのぞく",
    blurb: "Add the digits to reach one digit. Cast out 9s i.e. treat it as 0. If a sum is two digits, add again till it turns out single digit.",
    blurbJa: "各桁の数字をたして1桁にする。9は消す（0とみなす）。合計が2桁になったら、1桁になるまでもう一度たす。",
    methodTitle: "Procedure",
    methodTitleJa: "手順",
    steps: [
  ...
  },
  {
    ...
    sutraEnJa: "たして引く方法",
    blurb: "Answer in two parts — no multiplication! Left: the number − 1, then minus the rest digits (all except the units). Right: 10 − the units digit.",
    blurbJa: "かけ算なしで答えを2つの部分に分けます。左：数 − 1 から、残りの数字（一の位以外）を引く。右：10 − 一の位。",
    steps: [
  ...
  },
  {
    ...
    grad: ["#6D4C41", "#BCAAA4"],
    illus: "ladder",
    title: "Balancing Method – Thumb Rule",
    titleJa: "バランス法（親指のルール）",
    sutraSa: "Corollary · balancing",
    sutraEn: "Balancing Method – Thumb Rule",
    sutraEnJa: "バランス法",
    blurb:
      "Start from the right. Keep only the units digit in each segment, and carry the remaining digit(s) to the segment on the left. Continue until every segment, except the leftmost one, contains only one digit. Balancing is the final step in obtaining the answer.",
    blurbJa:
      "右から始めます。各区切りは一の位だけを残し、残りの数字を左の区切りへ送ります。いちばん左以外のすべての区切りが1桁になるまで続けます。バランスは答えを出す最後の手順です。",
  ...
  },
  {
    ...
    grad: ["#C2185B", "#F06292"],
    illus: "ladder",
    title: "Multiplication – Below Base 100",
    titleJa: "基準数法 — 100未満",
    sutraSa: "Nikhilam Navatáścaramam Daśataḥ",
    sutraEn: "deficiency from base 100",
    sutraEnJa: "基準100からの不足",
    blurb: "Find each number's deficiency from the base (100). Cross-subtract → left part. Multiply the deficiencies → right part (2 digits).",
    blurbJa: "それぞれの数の、基準（100）からの不足分を求める。たすきに引く → 左の部分。不足分どうしをかける → 右の部分（2桁）。",
    methodTitle: "Steps",
    methodTitleJa: "手順",
  ...
  },
  {
    ...
    grad: ["#5E35B1", "#9575CD"],
    illus: "ladder",
    title: "Multiplication – Above Base 100",
    titleJa: "基準数法 — 100より大きい",
    sutraSa: "Nikhilam Navatáścaramam Daśataḥ",
    sutraEn: "surplus over base 100",
    sutraEnJa: "基準100からの余り",
    blurb: "Find each number's surplus over the base (100). Cross-add → left part. Multiply the surpluses → right part (2 digits).",
    blurbJa: "それぞれの数の、基準（100）をこえた分を求める。たすきにたす → 左の部分。こえた分どうしをかける → 右の部分（2桁）。",
    methodTitle: "Steps",
    methodTitleJa: "手順",
  ...
  },
  {
    ...
    grad: ["#43A047", "#A5D6A7"],
    illus: "numberline",
    title: "Mental Addition – Complete the Ten First",
    titleJa: "暗算の足し算 ― まず10をつくる",
    sutraSa: "Sankalana-Vyavakalanabhyām",
    sutraEn: "The Bar Method",
    sutraEnJa: "バー（棒）法",
    blurb: "Make friendly tens: pair digits that add to 10. Group tens with tens, units with units — left to right.",
    blurbJa: "10になる友だちの数どうしを組にして、きりのいい10をつくる。十の位は十の位、一の位は一の位でまとめ、左から右へ。",
    methodTitle: "The Idea",
    methodTitleJa: "考え方",
  ...
  },
  {
    ...
    blurb: "If an upper digit is smaller, take the complement of the difference. Last digit → from 10; other digits → from 9. Reduce the left digit by 1 when needed.",
    blurbJa: "上の数字が小さいときは、差の補数をとる。最後の桁は10から、ほかの桁は9から。必要なときは左の桁を1減らす。",
    methodTitle: "Steps",
    methodTitleJa: "手順",
    steps: [
...

  {
    ...
    grad: ["#455A64", "#90A4AE"],
    illus: "ladder",
    title: "Subtraction (other than 10's Multiples)",
    titleJa: "4000・7000…からのひき算",
    sutraSa: "Corollary · Nikhilam",
    sutraEn: "reduce the leading digit by 1",
    sutraEnJa: "先頭の桁を1減らす",
    blurb: "For minuends like 4000/7000, drop the leading digit by 1. Apply All from 9 & Last from 10 (Nikhilam) to the remaining zeros.",
    blurbJa: "4000や7000のような数から引くときは、先頭の数字を1つ小さくする。残りの0の部分は「すべて9から、最後は10から」（ニキラム）で引く。",
    methodTitle: "The Trick",
    methodTitleJa: "コツ",
  ...
  },
  {
    ...
    grad: ["#D81B60", "#F48FB1"],
    illus: "squaregrid",
    title: "Square of a Number Starting with 5",
    titleJa: "5から始まる数の2乗",
    sutraSa: "Corollary · (a+b)² identity",
    sutraEn: "25 + units, then units squared",
    sutraEnJa: "25＋単位、そして単位の2乗",
    blurb: "Step 1: 5 × 5 = 25, then add the units digit once → first part. Step 2: units × units → second part (always 2 digits).",
    blurbJa: "手順1：5 × 5 = 25 に、一の位を1回たす → 最初の部分。手順2：一の位 × 一の位 → 2番目の部分（いつも2桁）。",
    steps: [
  ...
  /* p.131 — Addition, General Method: the Bar Method */
  {
    ...
    blurb: "Sub-base = the tens digit × 10 (e.g. 20, 30 … 90). Cross-subtract, then × the sub-base tens → left. Multiply deficiencies → right.",
    blurbJa: "準基準 ＝ 十の位 × 10（20、30…90）。たすきに引いて、準基準の十の位をかける → 左。不足分どうしをかける → 右。",
    methodTitle: "Steps",
    methodTitleJa: "手順",
    steps: [
  ...
  },
  {
    ...
    sutraEn: "Sub-base multiples of 10",
    sutraEnJa: "10の倍数のサブベース",
    blurb: "Sub-base = the tens digit × 10. Find each surplus over it. Cross-add, then × the sub-base tens → left. Multiply surpluses → right.",
    blurbJa: "準基準 ＝ 十の位 × 10。それぞれの数が準基準をこえた分を求める。たすきにたして、準基準の十の位をかける → 左。こえた分どうしをかける → 右。",
    methodTitle: "Steps",
    methodTitleJa: "手順",
    steps: [
      "Pick the sub-base: the tens digit × 10.",
  ...
  /* ---------- World 7 · duplex squares ---------- */
  {
    ...
    grad: ["#D89B00", "#FFCC00"],
    illus: "squaregrid",
    title: "Square by Duplex Method",
    titleJa: "デュープレックス法の2乗",
    sutraSa: "Dvandva Yoga · duplex",
    sutraEn: "(ab)² = D(a) | D(ab) | D(b)",
    sutraEnJa: "(ab)² = D(a) | D(ab) | D(b)",
    blurb: "D(a) = a²\nD(ab) = 2ab\nD(abc) = 2ac + b²\n2-digit: (ab)² = D(a) | D(ab) | D(b)\n3-digit: (abc)² = D(a) | D(ab) | D(abc) | D(bc) | D(c)\nBalance when 2 or more digits appear.",
    blurbJa: "D(a) = a²\nD(ab) = 2ab\nD(abc) = 2ac + b²\n2桁：(ab)² = D(a) | D(ab) | D(b)\n3桁：(abc)² = D(a) | D(ab) | D(abc) | D(bc) | D(c)\n2桁以上になったらバランスする。",
    methodTitle: "Duplex (D) definitions",
    methodTitleJa: "デュープレックス（D）の定義",
    steps: [
